...
## [2026-09-23] `/research` — sửa theo audit skill-creator + eval 3 case

- **Quyết định:** viết lại `research/SKILL.md`: (1) một cổng duyệt nội dung duy nhất (bước 2 — đề xuất gộp A+B+link+analysis+hệ quả status), thay cho cổng rời ở A và B; (2) đọc `00_schema.md` theo nhánh, chỉ khi sắp ghi (§7, §9, §10 nhiều file; +§1, §8 nếu analysis); (3) gom trang theo chunk, đọc trọn **mục** thay vì trọn chương; (4) chú thích nguồn nhiều file có hậu tố `file`; (5) nhánh mâu thuẫn nguồn → `⚠️ Conflict`, claim sai của chính nguồn → `_inbox.md`, khái niệm thiếu trang → gap C (không tạo stub); (6) link-only không nâng `last_updated`; (7) analysis vào `index.md`, `--all` bắt buộc khi có ghi trang; (8) research luôn ghi log, như lint. Đồng bộ `CLAUDE.md` (bảng operation, luật cứng 4, `--all`), description `query`, `.claude/rules/project-records.md`.
