...
- **Nguồn ngắn** (≤ 120 KB *và* ≤ 1.200 dòng, tính trên tổng các file): đọc trọn 1 lượt. Không cần file trạng thái.
...
**2. Phân nhóm theo chủ đề.** Cụm hoá kết quả của bước 1 — **không đọc lại nguồn**. Một chủ đề thường chứa **nhiều** trang atomic. Đối chiếu mỗi cụm với **toàn bộ** `02_wiki/` để tránh trùng trang đã có, bằng tên file và frontmatter, không mở thân bài:
...
Viết thân bài bằng lời của mình và áp skill `writing-style` (profile wiki). Với **nguồn dài**: mỗi claim kèm chú thích `(<source id>, <chương>, <mục>, d.<từ>–<đến>)` ngay sau claim (§7.5); nguồn nhiều file thêm `file <hậu tố>`. Dải dòng lấy từ chunk đang đọc.
...
**5. Ngưỡng:** 5–15 trang mỗi lần (§4). Trang `stub` không tính.

**6. Cập nhật `02_wiki/index.md`** — phần đầu **bắt buộc mỗi lượt** (luật cứng 6):
...
**7. Cập nhật `03_state/<source id>.md`** (nguồn dài). Chunk vừa xử lý sang `[x]`, hoặc `[~]` kèm ghi chú phần còn lại nếu dừng giữa chừng; cập nhật `last_updated`. Bảng này và §Sources phải khớp nhau.
...
- Bỏ qua bước 0 vì "nguồn đơn giản" → người dùng mất vai trò trong vòng lặp.
