...
python .claude/hooks/validate_wiki_page.py --backlinks   # bảng đếm backlink
```
...
**4. Ghi 1 mục vào cuối `log.md`** (§12): `## [<giờ>] promote | <số trang> trang lên stable`, dòng dưới liệt kê tên trang (hoặc số trang nếu quá dài) và các trang bị loại kèm lý do. Giờ lấy bằng `python .claude/hooks/validate_wiki_page.py --now`.
