---
name: writing-style
description: Bộ quy tắc văn phong tiếng Việt cho trang wiki và báo cáo kinh tế vĩ mô — chống "văn AI", ép chủ thể cụ thể, số liệu có nguồn, từ vựng kỷ luật. Tổng hợp từ ba nguồn: hard-rules nội bộ (báo cáo thanh khoản toàn cầu T4/2026), unslop guide (cursor/plugins), và mẫu BIS Quarterly Review. Các skill khác trích dẫn rule qua số ổn định (ví dụ "áp A1, C2"). Áp dụng mỗi khi viết hoặc sửa văn bản tiếng Việt trong project này — tạo trang wiki, merge nội dung khi ingest, trả lời query, sửa claim khi review.
---

# Quy tắc văn phong

Rà và sửa văn bản tiếng Việt (trang wiki, báo cáo) để loại dấu vết AI và giữ đúng giọng phân tích kinh tế nghiêm túc: chủ thể cụ thể, số liệu gắn nguồn, không ẩn dụ cực đoan, không câu nhồi nhét.
...
5. Nếu là trang `02_wiki/`: áp thêm ràng buộc ở cuối mục §F (profile wiki). Nếu là báo cáo/tài liệu ngoài wiki: áp profile báo cáo.
...
**B5. Cấm tính từ định tính đứng một mình không kèm số liệu:** *khổng lồ, cực đoan, nghiêm trọng, mong manh, tột độ, cực kỳ, hoàn toàn.* Kèm số ngay sau ("lên tới X tỷ", "ở mức X%, vượt ngưỡng Y%") hoặc xóa.
...
**C2. Mọi số liệu phải gắn nguồn hoặc kỳ quan sát cụ thể** — dạng "[X] theo [nguồn], tính đến [thời điểm]", hoặc nếu văn bản đã định nghĩa một "kỳ báo cáo" chung ở đầu bài thì có thể neo vào đó xuyên suốt. Với trang `02_wiki/`, đây chính là chú thích vị trí §7.5 đã có trong `00_schema.md` — rule này chỉ nhắc lại, không thêm nghĩa vụ mới.

**C3. Thuật ngữ nhóm/phạm vi phải có định nghĩa neo khi dùng lần đầu** nếu không hiển nhiên (ví dụ nhóm công ty nào tính là "hyperscaler", giai đoạn nào tính là "kỳ báo cáo"). Đặt ở chú thích cuối, không nhét vào câu phân tích chính.
...
**Profile wiki (`02_wiki/`):** trang phẳng, không heading trong thân bài (luật có sẵn trong `00_schema.md`, rule này không thêm gì mới, chỉ nối vào §E1: nếu một trang cần đổi giọng — ví dụ từ phân tích sang giải thích phổ thông — đó là dấu hiệu phải tách trang, không phải chèn heading hay hộp).

**Profile báo cáo/tài liệu ngoài wiki:**
...
Áp dụng cho nội dung skill, trang wiki, báo cáo, và câu trả lời của trợ lý khi làm việc trong project này. Khác bản chất với §G (hình thức trình bày): đây là cụm ngôn ngữ tự nói về quá trình viết/sửa hoặc ẩn dụ mơ hồ, không phải quy tắc trình bày kỹ thuật.
...
| giờ đã đầy đủ / hoàn chỉnh | tự đánh giá tiến độ, không mô tả gì cụ thể | xóa, hoặc nêu đúng phần còn thiếu nếu có |
...
| neo giữ / làm điểm neo / bám neo vào thực tế | cùng ẩn dụ lặp lại không cần thiết | "dựa trên [nguồn cụ thể]" |
