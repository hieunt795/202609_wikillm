# LLM Wiki — Macroeconomics · CLAUDE.md V4.1

> Bản thử nghiệm để review thủ công. Claude Code không tự nạp file này hoặc
> `.claude/rules-draft/`; `CLAUDE.md` hiện hành vẫn có hiệu lực.
...
| `.claude/skills/*/SKILL.md` | Quy trình thực thi duy nhất cho từng operation |
...
- Rule chuyên biệt dự kiến: `.claude/rules-draft/`; không dùng `@import`.
...
Khi kích hoạt, review rồi đổi `rules-draft` thành `rules`. Không sao chép toàn bộ
schema hoặc skill vào rules. Giữ file luôn-nạp này dưới 120 dòng.

## Session handoff

Phiên làm thay đổi repo hoặc trạng thái vận hành phải tạo đúng một file mới:
`session_handoffs/YYYY-MM-DD-HHmm-<short-slug>.md`. Phiên chỉ đọc, khảo sát hoặc
hỏi đáp không thay đổi trạng thái thì không tạo. Chi tiết ở rule handoff.
